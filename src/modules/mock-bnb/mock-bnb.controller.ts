...
export class MockBnbController {
  async simulatePayment(req: Request, res: Response): Promise<void> {
    try {
      ...
      const payload = {
        ...
        transactionNumber: `BNB-TX-${Math.floor(100000 + Math.random() * 900000)}`,
      }
      ...
      const processingResult = await webhooksService.processBnbNotification(
        payload as any,
      )

      res.status(200).json({
        simulatedEvent: 'WEBHOOK_DISPATCHED',
